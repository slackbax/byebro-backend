$(document).ready(function () {
    ...
    function showResponse(response) {
        ...
        if (response.type) {
            ...
            $('div.has-feedback').each(function () {
                $(this).removeClass('has-error has-success');
            });

            $('i.form-control-feedback').each(function () {
                $(this).removeClass('fa-remove fa-check');
            });
            $('#formNewQuota').clearForm();
            $('select').each(function () {
                $(this).prop("selectedIndex", 0);
            });
            $('#iNisuser').prop('checked', false).iCheck('update');

            var today = new Date(), tomorrow = new Date(), dayafter = new Date();
            tomorrow.setDate(today.getDate() + 1);
            dayafter.setDate(tomorrow.getDate() + 1);

            $('#daterange').daterangepicker({startDate: tomorrow, endDate: dayafter}).val(tomorrow + ' hasta ' + dayafter);
            //getCountries();
        } else {
            if (response.code === 0) {
                new Noty({
    ...
    var options = {
        url: 'quotations/ajax.insertQuotation.php',
        type: 'post',
        dataType: 'json',
        beforeSubmit: validateForm,
        success: showResponse
    };
    var nParticipantes = 2, nTotal = 1;
    ...
    $('#iNisuser').on('ifChanged', function () {
        if ($(this).prop('checked')) {
            $.ajax({
                type: 'POST',
                url: 'quotations/ajax.getUserData.php',
                dataType: 'json',
                data: {id: $('#usid').val()}
            }).done(function (r) {
                if (r.co_id !== '') {
    ...
    $('#iNocountry, #iNdcountry, #iNocity, #iNdcity, #iNname, #iNlastnamep, #iNlastnamem, #iNemail, #iNphone').change(function () {
    ...
    /*
        function fillForm() {
            $('#iNocity').val(1).change();
            $('#iNdcity').val(2).change();
            //$('#iNisuser').iCheck('check');
            $('#iNrutpart1').val('15.652.908-7').change();
            $('#iNedadpart1').val('19').change();
            $('#iNnamepart1').val('Nacho').change();
            $('#iNlnppart1').val('Muñoz').change();
            $('#iNlnmpart1').val('Jensen').change();
            $('#iNemailpart1').val('i.munoz.j@gmail.com').change();
            $('#iNphonepart1').val('999195279').change();
        }
