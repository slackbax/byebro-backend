...
$columns = array(
    ...
    array('db' => 'v.vi_id', 'dt' => 7, 'field' => 'vi_id',
        'formatter' => function ($d, $row) use ($per) {
            return count($per->getByViaje($d));
        }
    ),
    ...
    array('db' => 'v.vi_id', 'dt' => 9, 'field' => 'vi_id',
        'formatter' => function ($d, $row) use ($via, $_admin) {
            ...
            if ($_admin):
                $string .= ' <button id="id_' . $d . '" class="assignStaff btn btn-xs btn-success" data-toggle="modal" data-target="#modal-staff" data-tooltip="tooltip" data-placement="top" title="Asignar personal"><i class="fa fa-user-plus"></i></button>';
            endif;
...
$extraWhere = " vies.vie_ultimo IS TRUE AND vies.ves_id IN (1,2,5,6)";
$groupBy = "";
$having = "";
