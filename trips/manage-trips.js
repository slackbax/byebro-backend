$(document).ready(function () {
    var tableQuot = $("#ttrips").DataTable({
        ...
        serverSide: true,
        ajax: {
            url: 'trips/ajax.getServerTrips.php',
            type: 'GET',
            length: 20
        }
    });
    ...
    }).on('click', '.tripValue', function () {
        var uid = $(this).attr('id').split("_").pop();

        swal({
            title: "Ingresa el valor para el viaje:",
            input: 'text',
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "Sí",
            inputValidator: (value) => {
                return !value && '¡Debes ingresar un valor para el viaje!'
            },
            preConfirm: (input) => {
                $.ajax({
                    url: 'trips/ajax.setValue.php',
                    type: 'POST',
                    dataType: 'json',
    ...
    }).on('click', '.tripReject', function () {
        var uid = $(this).attr('id').split("_").pop();
        $(this).parent().parent().addClass('selected');

        swal({
            title: "¿Estás seguro de querer rechazar el viaje?",
            text: "Esta acción dejará el viaje en estado RECHAZADO.",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "Sí"
