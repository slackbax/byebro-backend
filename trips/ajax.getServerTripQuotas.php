...
$columns = array(
    ...
    array('db' => 'v.vi_id', 'dt' => 9, 'field' => 'vi_id',
        'formatter' => function ($d, $row) use ($via, $_admin) {
            ...
            if ($_admin):
                if ($ves->ves_id == 1):
                    $string .= ' <a href="index.php?section=trips&sbs=assignquota&id=' . $d . '" class="tripValue btn btn-xs btn-success" data-tooltip="tooltip" data-placement="top" title="Asignar cuotas"><i class="fa fa-check-square-o"></i></a>';
                endif;
            endif;
...
$extraWhere = " vies.vie_ultimo IS TRUE AND v.vi_valor IS NOT NULL AND vies.ves_id = 1";
$groupBy = "";
$having = "";
