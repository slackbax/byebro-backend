...
if (extract($_POST)):
    ...
    try {
        $db->autoCommit(FALSE);
        $ins = $p->mod($id, $iname, $ilastnamep, $ilastnamem, $iemail, $iphone, $db);
        ...
        if (!empty($iusernameid)):
            $ins_us = $p->modUser($id, $iusernameid, $db);
        ...
        $del_c = $p->delCities($id, $db);

        if (!$del_c['estado']):
            throw new Exception('Error al eliminar los datos de las ciudades del personal. ' . $del_c['msg'], 0);
        endif;

        foreach ($iid as $i => $val):
            if (!empty($val)):
                $ins_c = $p->setCity($id, $val, $db);
